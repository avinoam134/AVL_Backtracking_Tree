...
public class BTree<T extends Comparable<T>> {
    ...
    public void insert(T value) {
        //adds a signal for the backtrack for each insertion
        stack.add("END SESSION");
        if (root == null) {
            Node<T> newRoot = new Node<T>(null, maxDegree);
            newRoot.addKey(value);
            root = newRoot;
        }
        else {
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                    //adds the median value to the stack
                    stack.push(split(currentNode));
    ...
    private Node<T> DuplicateNode(Node<T> origin) {
        Node<T> copy = new Node<T>(null, maxDegree);

        for (int i = 0; i < origin.numOfKeys; ++i) {
            copy.keys[i] = origin.keys[i];
        }

        if (origin.getNumberOfChildren() > 0) {
            for (int j = 0; j < origin.numOfChildren; ++j) {
                copy.children[j] = origin.children[j];
            }
        }

        copy.numOfKeys = origin.numOfKeys;
        copy.numOfChildren = origin.numOfChildren;
        copy.comparator = origin.comparator;
